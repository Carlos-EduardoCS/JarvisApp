...
import { meetingLink } from './tasks.ts';
...
export function parseBackup(text: string): Backup {
  ...
  const tasks = data.tasks.map((t: any): Task => {
    ...
    if (t.location !== undefined && (typeof t.location !== 'string' || t.location.length > 500)) throw new Error('O backup contém um endereço inválido.');
    if (t.meetingUrl !== undefined && typeof t.meetingUrl !== 'string') throw new Error('O backup contém um link de reunião inválido.');
    if (t.isMeeting !== undefined && typeof t.isMeeting !== 'boolean') throw new Error('O tipo da tarefa é inválido.');
    const meetingUrl = t.meetingUrl === undefined ? undefined : meetingLink(t.meetingUrl);
    ids.add(t.id);
    return { id: t.id, title: t.title, notes: t.notes, done: t.done, minutesBefore: t.minutesBefore, dueAt: new Date(t.dueAt).toISOString(), ...(t.location !== undefined ? { location: t.location.trim() } : {}), ...(meetingUrl !== undefined ? { meetingUrl: t.isMeeting === false ? '' : meetingUrl } : {}), ...(t.isMeeting !== undefined ? { isMeeting: t.isMeeting } : {}) };
  });
