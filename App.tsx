...
import { AccessibilityInfo, Alert, Animated, Easing, FlatList, KeyboardAvoidingView, Linking, Modal, Pressable, ScrollView, StyleSheet, Switch, Text, TextInput, useColorScheme, View } from 'react-native';
import { SafeAreaProvider, SafeAreaView } from 'react-native-safe-area-context';
import AsyncStorage from '@react-native-async-storage/async-storage';
import DateTimePicker from '@react-native-community/datetimepicker';
import { StatusBar } from 'expo-status-bar';
import { Task, reminderDate, visibleTasks, meetingLink, mapLink } from './src/tasks';
...
export default function App() {
  ...
  useEffect(() => {
    AsyncStorage.getItem(KEY).then(raw => {
      ...
      if (!Array.isArray(data) || !data.every(t => typeof t.id === 'string' && typeof t.title === 'string' && typeof t.notes === 'string' && typeof t.done === 'boolean' && Number.isFinite(t.minutesBefore) && Number.isFinite(Date.parse(t.dueAt)) && (t.location === undefined || (typeof t.location === 'string' && t.location.length <= 500)) && (t.isMeeting === undefined || typeof t.isMeeting === 'boolean') && (t.meetingUrl === undefined || (typeof t.meetingUrl === 'string' && t.meetingUrl.length <= 4096)))) throw new Error('Invalid data');
  ...
  function showForm(task?: Task) {
    setEditing(task ?? null); setTitle(task?.title ?? ''); setNotes(task?.notes ?? ''); setLocation(task?.location ?? ''); setMeetingUrl(task?.meetingUrl ?? ''); setIsMeeting(task?.isMeeting ?? !!task?.meetingUrl);
  ...
  async function save() {
    ...
    try { normalizedMeeting = isMeeting ? meetingLink(meetingUrl) : ''; } catch (error) { return Alert.alert('Confira o link', error instanceof Error ? error.message : 'Link inválido.'); }
    const task: Task = { id: editing?.id ?? `lembra-${Date.now()}-${Math.random().toString(36).slice(2)}`, title: title.trim(), notes: notes.trim(), location: location.trim(), meetingUrl: normalizedMeeting, isMeeting, dueAt: date.toISOString(), minutesBefore: minutes, done: editing?.done ?? false };
  ...
  async function openDestination(value: string, kind: 'map' | 'meeting') {
    try { await Linking.openURL(kind === 'map' ? mapLink(value) : meetingLink(value)); }
    catch { Alert.alert('Não foi possível abrir', kind === 'map' ? 'Confira o endereço e tente novamente.' : 'Confira o link da reunião e tente novamente.'); }
  }
  ...
  return <SafeAreaProvider><View style={s.page}><SafeAreaView style={s.page} pointerEvents={opening ? "none" : "auto"} accessibilityElementsHidden={opening} importantForAccessibility={opening ? "no-hide-descendants" : "auto"}><StatusBar animated style={isDark ? "light" : "dark"} />
    ...
    <FlatList data={list} keyExtractor={t => t.id} contentContainerStyle={s.list} ListEmptyComponent={ready ? <View style={s.empty}><Text style={s.sun}>☀</Text><Text style={s.heading}>{filter === 'Concluídas' ? 'Cada passo conta.' : 'Espaço para o que importa.'}</Text><Text style={[s.muted, { textAlign: 'center', marginTop: 12 }]}>{filter === 'Concluídas' ? 'Suas tarefas concluídas aparecerão aqui.' : 'Adicione uma tarefa e escolha quando receber o lembrete.'}</Text></View> : null} renderItem={({ item }) => <View style={s.card}>
      <Pressable disabled={busy} accessibilityLabel={`Editar ${item.title}`} onPress={() => showForm(item)}><Text style={[s.task, item.done && { textDecorationLine: 'line-through', opacity: 0.6 }]}>{item.title}</Text><Text style={s.meta}>{new Date(item.dueAt).toLocaleString('pt-BR', { day: '2-digit', month: '2-digit', hour: '2-digit', minute: '2-digit' })}{!item.done && new Date(item.dueAt) < now ? ' · Atrasada' : ''}</Text>{item.notes ? <Text style={s.muted}>{item.notes}</Text> : null}{!!item.location && <Text style={[s.muted, { marginTop: 10 }]}>Local: {item.location}</Text>}</Pressable>
      {(!!item.location || !!item.meetingUrl) && <View style={s.destinationActions}>
        {!!item.location && <Pressable accessibilityRole="link" accessibilityLabel={`Abrir no mapa: ${item.location}`} style={s.destinationButton} onPress={() => openDestination(item.location!, 'map')}><Text style={s.actionText}>Abrir no mapa</Text></Pressable>}
        {!!item.meetingUrl && <Pressable accessibilityRole="link" accessibilityLabel={`Entrar na reunião: ${item.title}`} style={s.destinationButton} onPress={() => openDestination(item.meetingUrl!, 'meeting')}><Text style={s.actionText}>Entrar na reunião</Text></Pressable>}
      </View>}
    ...
    <ProfileMenu visible={menuOpen} onClose={() => setMenuOpen(false)} onBackup={() => setBackupOpen(true)} colors={colors} dark={isDark} themeMode={themeMode} themeDisabled={!themeReady || themeSaving} onThemeChange={chooseTheme} />
    ...
    <Modal visible={open} animationType="slide" presentationStyle="pageSheet" onRequestClose={() => !busy && setOpen(false)}><SafeAreaView style={s.page}><KeyboardAvoidingView style={{ flex: 1 }} behavior="padding"><ScrollView contentContainerStyle={s.form} keyboardShouldPersistTaps="handled">
      ...
      <Text style={s.label}>Local ou endereço (opcional)</Text><TextInput accessibilityLabel="Local ou endereço" style={s.input} placeholderTextColor={colors.muted} selectionColor={colors.text} keyboardAppearance={isDark ? 'dark' : 'light'} placeholder="Av. Paulista, 1000, São Paulo" value={location} onChangeText={setLocation} maxLength={500} />
      <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginTop: 20, gap: 12 }}><Text style={[s.actionText, { flex: 1 }]}>É uma reunião?</Text><Switch accessibilityLabel="É uma reunião?" value={isMeeting} onValueChange={setIsMeeting} trackColor={{ false: isDark ? '#66758B' : '#8A97A8', true: isDark ? '#B49764' : '#142235' }} ios_backgroundColor={isDark ? '#66758B' : '#8A97A8'} thumbColor="#FFFFFF" /></View>
      {isMeeting && <><Text style={s.label}>Link da reunião (opcional)</Text><TextInput accessibilityLabel="Link da reunião" style={s.input} placeholderTextColor={colors.muted} selectionColor={colors.text} keyboardAppearance={isDark ? 'dark' : 'light'} placeholder="https://meet.google.com/..." value={meetingUrl} onChangeText={setMeetingUrl} autoCapitalize="none" autoCorrect={false} keyboardType="url" maxLength={4096} /><Text style={s.helper}>Cole o link do Meet, Teams ou Zoom. Para reuniões presenciais, deixe em branco.</Text></>}
...
const createStyles = (colors: typeof palettes.light) => StyleSheet.create({
  destinationActions: { flexDirection: 'row', flexWrap: 'wrap', gap: 8, marginTop: 14 },
  destinationButton: { minHeight: 44, paddingHorizontal: 12, paddingVertical: 12, borderRadius: 10, backgroundColor: colors.subtle },
